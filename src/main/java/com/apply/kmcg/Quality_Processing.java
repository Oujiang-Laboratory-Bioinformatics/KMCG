...
//
//    public static List<List<Integer>> readAndProcessFile(String absolutePath) {
...
//                }
//                else if (lineNumber == 522) {
//                    // 读取第522行数据并存入 quality_indication
//                    if (tokens.length == 1) {
//                        try {
//                            double value = Double.parseDouble(tokens[0]);
//                            quality_indication.add(value); // 将第522行的单个值添加到 quality_indication
//                        } catch (NumberFormatException e) {
//                            System.err.println("[WARN] Invalid double in line " + lineNumber + ": " + tokens[0]);
//                            quality_indication.add(null); // 保留 null 占位
//                        }
//                    } else {
//                        System.err.println("[WARN] Line " + lineNumber + " contains more than one token, skipping.");
//                    }
//                }
//
//
...
// **在折线绘制完成后再画红色空心圆圈**
        ...
        gc.fillText(qualityIndication, (canvasWidth - gc.getFont().getSize() * qualityIndication.length()) / 2 + 650, canvasHeight+20);  // 质量值也向右移动500
