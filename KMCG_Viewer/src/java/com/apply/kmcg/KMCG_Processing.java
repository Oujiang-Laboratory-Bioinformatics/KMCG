...
//    public static Map<String, List<List<Integer>>> scaffolddataMap = new HashMap<>();
    ...
    static String normalizeLine(String line) {
        ...
        return line.replace("\r", "").replace('\u00A0', ' ').strip();
    }

    static boolean isExtensionMarker(String raw) {
    ...
    static boolean isScaffoldHeaderLine(String raw) {
        if (raw.isEmpty() || isExtensionMarker(raw) || isQualityLine(raw)) {
            return false;
        }
    ...
    static String formatScaffoldTagName(String raw) {
        return raw.trim().replace("\t", " ").replaceAll("\\s+", " ").strip();
    }
    ...
    static String parseScaffoldBlockFixed(BufferedReader br, String tagName) throws IOException {
        ...
        for (int i = 0; i < SCAFFOLD_BLOCK_DATA_ROWS; i++) {
            String dataLine = br.readLine();
            if (dataLine == null) {
                break;
            }
            String raw = normalizeLine(dataLine);
            if (!raw.isEmpty()) {
        ...
        if (!tagName.isEmpty() && !dataBlock.isEmpty()) {
            ...
            System.err.println("KMCG extension: parsed scaffold block '" + tagName + "' rows=" + dataBlock.size());
        } else {
            System.err.println("KMCG extension: skipped scaffold block '" + tagName + "' (no numeric rows)");
        }
        if (separator == null) {
            return null;
        }
        if (!normalizeLine(separator).isEmpty()) {
            return separator;
        }
        return br.readLine();
    }

    static String parseGcBlockFixed(BufferedReader br, String tagName) throws IOException {
        List<Integer> gcTemp = new ArrayList<>();
        String yLine = br.readLine();
        if (yLine != null) {
            gcTemp.addAll(parseIntegerRow(normalizeLine(yLine)));
        }
        String separator = br.readLine();
        if (!tagName.isEmpty() && !gcTemp.isEmpty()) {
            gcDataMap.put(tagName, gcTemp);
            System.err.println("KMCG extension: parsed gc block '" + tagName + "' points=" + gcTemp.size());
        }
        if (separator == null) {
            return null;
        }
    ...
    static void parseExtensionTail(BufferedReader br, String firstLine) throws IOException {
        ...
        while (current != null) {
            current = parseExtensionLine(br, current);
        }
    }
    ...
    static String parseExtensionLine(BufferedReader br, String line) throws IOException {
        ...
        String marker = raw.replace("\ufeff", "").strip();
        if (marker.startsWith("#")) {
            extensionBlocksStarted = true;
            return parseScaffoldBlockFixed(br, marker.substring(1).trim());
        }
        if (marker.startsWith("*")) {
            extensionBlocksStarted = true;
            return parseGcBlockFixed(br, marker.substring(1).trim());
        }
        if (isScaffoldHeaderLine(raw)) {
            extensionBlocksStarted = true;
            return parseScaffoldBlockFixed(br, formatScaffoldTagName(raw));
        }
        if (!extensionBlocksStarted) {
            return br.readLine();
        }
    ...
    public static List<List<Integer>> readFile(String filepath) {
        ...
        try {
            ...
            if (isGzip) fileStream = new GZIPInputStream(fileStream);
            ...
            try (BufferedReader br = new BufferedReader(new InputStreamReader(fileStream, StandardCharsets.UTF_8))) {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    if (lineNumber == 1) {
                        ...
                        logExtensionDebug(filepath, lineNumber, raw);
                        parseExtensionTail(br, line);
                        break;
                    }
                }
        ...
        if (scaffolddataMap.isEmpty() && gcDataMap.isEmpty()) {
            System.err.println("KMCG extension: no #/* blocks parsed after line 1133 (lineNumber reached end of fixed header)");
        } else {
            System.err.println("KMCG extension: scaffold tabs=" + scaffolddataMap.size() + ", gc tabs=" + gcDataMap.size());
        }
