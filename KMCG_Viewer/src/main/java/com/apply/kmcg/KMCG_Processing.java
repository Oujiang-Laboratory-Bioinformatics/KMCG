...
//                showDataTooltip(dataCanvas, data, "region1",  mouseX, mouseY);  // 显示第一行的第一列
                ...
                //region5的坐标位置
                showRegion5Coordinates(dataCanvas, data, mouseX, mouseY-30, isPolyploidView); // 显示 region5 坐标
        ...
        if (kmerCanvas != null) {
            ...
            kmerCanvas.setOnMouseMoved(event -> {
                ...
                if (isMouseInBlock) {
                    ...
                    double tooltipX = mouseX - (textWidth + padding * 2) / 2; // 水平居中
                    if (tooltipX < 0) tooltipX = 0; // 防止超出左边界
                    if (tooltipX + textWidth + padding * 2 > kmerCanvas.getWidth())
                        tooltipX = kmerCanvas.getWidth() - textWidth - padding * 2; // 防止超出右边界

                    double tooltipY = Math.max(mouseY - textHeight - padding * 2 - 5, 0); // 鼠标正上方，留点间距
