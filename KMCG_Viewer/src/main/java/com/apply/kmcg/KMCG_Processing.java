...
public class KMCG_Processing {
    ...
    private static boolean hoverTooltipsSuspended = false;

    public static void setHoverTooltipsSuspended(boolean suspended) {
        hoverTooltipsSuspended = suspended;
        if (suspended && currentTooltip != null) {
            currentTooltip.hide();
        }
    }

    static boolean areHoverTooltipsSuspended() {
        return hoverTooltipsSuspended;
    }
  ...
  /** Canvas rectangle for one KMCG matrix cell (includes margin offsets). */
    ...
    private static final double INF_MU_LINE_WIDTH = 3.0;
    private static final Pattern INF_MU_PATTERN = Pattern.compile(
            "(?:μ|µ|mu)\\s*[:=]\\s*([\\d.]+)", Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);

    /** μ from standalone 1133-style line (μ:… σ:…), used for # tab reference line only. */
    public static Double infReferenceMu = null;

    static Double resolveInfMuValue() {
        if (infReferenceMu != null) {
            return infReferenceMu;
        }
        for (String line : quality_indication) {
            Matcher matcher = INF_MU_PATTERN.matcher(line);
            if (matcher.find()) {
    ...
    static double[] kmcgGraphCoordToCanvas(double graphX, double graphY, int rowCount, int colCount) {
        graphX = Math.max(0, Math.min(colCount - 1, graphX));
        graphY = Math.max(0, Math.min(rowCount - 1, graphY));
        int xBase = (int) Math.floor(graphX);
        int yBase = (int) Math.floor(graphY);
        double fx = graphX - xBase;
        double fy = graphY - yBase;
        KmcgCellRect rect = computeKmcgCellRect(yBase, xBase, rowCount, colCount);
        double canvasX = rect.x + fx * rect.width;
        double canvasY = rect.y + rect.height - fy * rect.height;
        return new double[]{canvasX, canvasY};
    }

    static KmcgCellRect interiorHeatmapCanvasRect(int rowCount, int colCount) {
        KmcgCellRect topLeft = computeKmcgCellRect(1, 1, rowCount, colCount);
        KmcgCellRect bottomRight = computeKmcgCellRect(rowCount - 2, colCount - 2, rowCount, colCount);
        return new KmcgCellRect(
                topLeft.x,
                topLeft.y,
    ...
    static double[] computeInfMuLineStart(double mu, int rowCount, int colCount) {
        double slope = mu / 10.0;
        if (slope <= 0) {
    ...
    static double[] computeInfMuLineEnd(double mu, int rowCount, int colCount) {
        double slope = mu / 10.0;
        if (slope <= 0) {
            return new double[]{0, 0};
        }
        double xMax = colCount - 2;
        double yMax = rowCount - 2;
        double endY = Math.min(yMax, xMax / slope);
        double endX = slope * endY;
        if (endX > xMax) {
            endX = xMax;
            endY = xMax / slope;
        }
    ...
    /** White reference line x = (mu/10)*y; sampled in graph space so non-uniform cells stay aligned with tooltip coords. */
    static void drawInfMuReferenceLine(GraphicsContext gc, int rowCount, int colCount) {
        Double mu = resolveInfMuValue();
        if (mu == null) {
            return;
        }
        double slope = mu / 10.0;
        ...
        double[] end = computeInfMuLineEnd(mu, rowCount, colCount);
        if (end[1] <= 0 && end[0] <= 0) {
            return;
        }
        KmcgCellRect clip = interiorHeatmapCanvasRect(rowCount, colCount);
        gc.save();
        gc.beginPath();
        gc.rect(clip.x, clip.y, clip.width, clip.height);
        gc.clip();
        gc.setStroke(Color.WHITE);
        gc.setLineWidth(INF_MU_LINE_WIDTH);
        gc.beginPath();
        double[] origin = kmcgGraphCoordToCanvas(0, 0, rowCount, colCount);
        gc.moveTo(origin[0], origin[1]);
        int maxY = (int) Math.floor(end[1]);
        for (int gy = 1; gy <= maxY; gy++) {
            double gx = slope * gy;
            double[] pt = kmcgGraphCoordToCanvas(gx, gy, rowCount, colCount);
            gc.lineTo(pt[0], pt[1]);
        }
        double[] endCanvas = kmcgGraphCoordToCanvas(end[0], end[1], rowCount, colCount);
        gc.lineTo(endCanvas[0], endCanvas[1]);
        gc.stroke();
        gc.restore();
    }

    /** Temporary debug label — shows resolved μ above # tab heatmap (region8). */
    static void drawInfMuDebugLabel(GraphicsContext gc) {
        Double mu = resolveInfMuValue();
        String text = mu != null ? String.format("μ: %.4f", mu) : "μ: (未读取)";
        ...
        double labelY = region8.getY() + (region8.getHeight() + EXTENSION_BLOCK_LABEL_FONT_SIZE) / 2 - 2;
...
//    public static Map<String, List<List<Integer>>> scaffolddataMap = new HashMap<>();
  ...
  /** Returns [tagName, mode, displayText] where mode is "inf" (#) or "prm" (%). */
    ...
    static boolean isStandaloneQualityMetadataLine(String raw) {
        ...
        if (isQualityLine(raw)) {
            return true;
        }
        boolean hasMu = INF_MU_PATTERN.matcher(raw).find();
        boolean hasSigma = Pattern.compile("(?:σ|sigma)\\s*[:=]\\s*[\\d.]+",
                Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE).matcher(raw).find();
        return hasMu && hasSigma;
    }
    ...
    static void parseQualityLine(String line) {
        qualityMetadataRawLines.add(line);
        ...
        Matcher kPlusMatcher = kPlusPattern.matcher(normalized);
        if (kPlusMatcher.find()) {
            qualityKPlus = Double.parseDouble(kPlusMatcher.group(1));
        }
        Matcher kMinusMatcher = kMinusPattern.matcher(normalized);
        if (kMinusMatcher.find()) {
            qualityKMinus = Double.parseDouble(kMinusMatcher.group(1));
        }
