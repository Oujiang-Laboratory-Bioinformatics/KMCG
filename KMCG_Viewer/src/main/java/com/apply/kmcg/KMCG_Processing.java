...
//        System.out.println(resultData);
    ...
    public static String formatWithUnit(double value) {
        if (value >= 1_000_000) {
            ...
            return (v == (int)v) ? String.format("%dM", (int)v) : String.format("%.2fM", v);
        } else if (value >= 1_000) {
            double v = value / 1_000.0;
            return (v == (int)v) ? String.format("%dK", (int)v) : String.format("%.2fK", v);
        } else {
            return (value == (int)value) ? String.format("%d", (int)value) : String.format("%.2f", value);
        }
    }
    ...
    public static void drawToKmerCanvas(Map<String, List<Integer>> updatedStorage, double maxValue, double totalValue) {
        ...
        if (kmerCanvas != null) {
            ...
            double rowSpacing = adaptiverectHeight + 20;
            double topOffset = 40 + 40 + 15;

            double totalHeight = topOffset + updatedStorage.size() * rowSpacing + 50;
            ...
            for (Map.Entry<String, List<Integer>> entry : updatedStorage.entrySet()) {
                ...
                gc.fillText(name, colorBarX, yPos + 18);

                double xPosRect = colorBarX + extra_spacing;

                for (int i = 0; i < values.size(); i++) {
                    int value = values.get(i);
                    double blockWidth = rectWidth;

                    gc.setFill(ColorUtils.getColorForValue(value, maxValue, "UD"));
                    gc.fillRect(xPosRect, yPos, blockWidth, rectHeight);
                    xPosRect += blockWidth;

                    gc.setFill(Color.TRANSPARENT);
                    gc.fillRect(xPosRect - blockWidth, yPos, blockWidth, rectHeight);
                }
            ...
            kmerCanvas.setOnMouseMoved(event -> {
                ...
                for (Map.Entry<String, List<Integer>> entry : updatedStorage.entrySet()) {
                    ...
                    for (int i = 0; i < values.size(); i++) {
                        double blockXPos = currentXPos + i * rectWidth;
                        double blockEndX = blockXPos + rectWidth;
                        ...
                        if (mouseX >= blockXPos && mouseX <= blockEndX && mouseY >= currentYPos && mouseY <= currentYPos + rectHeight) {
                            ...
                            displayText = formatWithUnit(start) + " ~ " + formatWithUnit(end);
                            isMouseInBlock = true;
                            break;
                        }
                    }
                ...
                if (isMouseInBlock) {
                    ...
                    double textHeight = gc.getFont().getSize() + 6;

                    double tooltipX = Math.min(mouseX, kmerCanvas.getWidth() - textWidth - padding * 2);
                    double tooltipY = Math.max(mouseY - textHeight - 10, 0);
            ...
            kmerCanvas.setOnMouseClicked(event -> {
                ...
                if (isMouseInBlock && blockValue != -1) {
                    ...
                    content.append(String.format("Row: %d, position: %s ~ %s\nName: %s\nCount: %d (%s)\n\n",
                            rowIndex, formatWithUnit(start), formatWithUnit(end), clickedName, blockValue, String.format("%.3f%%", percentage)));
