...
public class KMCG_Processing {
    ...
    // 覆盖块的位置
    public static Rectangle region1 = new Rectangle(0 + Marginx_KMCG, 674 + Marginy_KMCG, 31, 31); // x, y, width,
                                                                                                   // height
    ...
    // public static Map<String, List<List<Integer>>> scaffolddataMap = new
    // HashMap<>();
    // 构造函数接收 Canvas 实例
                ...
                if (lineNumber == 1) { // 第一行，跳过的实际上是第一行
                    ...
                    if (tokens.length == 3) {
                        ...
                        try {
                            ...
                            System.err.println(
                                    "Error: Invalid number format in line 1. Height and width should be integers.");
                        }
        ...
        for (int y = 0; y < rowCount; y++) {
            ...
            for (int x = 0; x < colCount; x++) {
                ...
                if (value != null) {
                    ...
                    gc.setFill(color);
                    gc.fillRect(xPos + Marginx_KMCG, yPos + Marginy_KMCG, colScaleX, rowScaleY); // 绘制矩形，使用动态的 colScaleX
                                                                                                 // 和 rowScaleY
                }
            }
    ...
    public static void showDataTooltip(Canvas dataCanvas, List<List<Integer>> data, Object regionIdentifier,
            ...
            if (region5.contains(mouseX, mouseY)) {
                ...
                if (clickedPoints.size() == 4) {
                    ...
                    if (canFormQuadrilateral(quadrilateralPoints)) {
                        ...
                        Map<String, List<Integer>> storage = Kmer_Processing
                                .initializeStorage(Kmer_Processing.NamesData(), Kmer_Processing.LengthsData());
                        Map<String, List<Integer>> updatedStorage = Kmer_Processing
                                .updateStorage(Kmer_Processing.NamesData(), storage, parsedData);

                        int MaxValue = Kmer_Processing.getMaxValue(Kmer_Processing.NamesData(), storage, parsedData)
                                + 1;
                        int getTotalValue = Kmer_Processing.getTotalValue(Kmer_Processing.NamesData(), storage,
                                parsedData);
                        drawToKmerCanvas(updatedStorage, MaxValue, getTotalValue);

                    } else {
                        // 弹出提示框提示用户无法构成四边形
                        showAlert("Cannot form a quadrilateral",
                                "The selected points cannot form a valid quadrilateral, please reselect the points。");
    ...
    public static void showDefaultImage(Canvas dataCanvas) {
        ...
        Image defaultImage = new Image(
                Objects.requireNonNull(KMCG_Processing.class.getResourceAsStream("/image/default_image.png")));
        // 在 Canvas 上绘制默认图片
        GraphicsContext gc = dataCanvas.getGraphicsContext2D();
        gc.clearRect(0, 0, dataCanvas.getWidth(), dataCanvas.getHeight()); // 清空画布
            ...
            kmerCanvas.setOnMouseMoved(event -> {
                ...
                for (Map.Entry<String, List<Integer>> entry : updatedStorage.entrySet()) {
                    ...
                    for (int i = 0; i < values.size(); i++) {
                        ...
                        if (mouseX >= blockXPos && mouseX <= blockEndX && mouseY >= currentYPos
                                && mouseY <= currentYPos + rectHeight) {
                            // 如果鼠标在块内，计算并显示块的区间
