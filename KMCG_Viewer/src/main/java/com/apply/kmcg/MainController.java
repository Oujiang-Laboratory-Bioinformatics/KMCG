...
public class MainController {
    ...
    /** Fraction of K+ and K- from quality metadata line (1133), e.g. 0.07 and 0.05 */
    public static Double qualityKPlus = null;
    public static Double qualityKMinus = null;
    /** Unmodified quality metadata lines (1133-style) for parameter lookup */
    public static List<String> qualityMetadataRawLines = new ArrayList<>();
    ...
    @FXML
    private StackPane qualityFittingPane;
    @FXML
    private GridPane qualityLayoutGrid;
    @FXML
    private GridPane qualityRightGrid;
    @FXML
    private StackPane qualityAssemblyPane;
    @FXML
    ...
    @FXML
    private Canvas qualityAssemblyCanvas;     // 装配比例条
    ...
    public void initialize() {
        qualityFittingPane.setVisible(false);
        if (qualityLayoutGrid != null && qualityLayoutGrid.getRowConstraints().size() > 0) {
            RowConstraints outerRow = qualityLayoutGrid.getRowConstraints().get(0);
            outerRow.setMinHeight(754);
            outerRow.setPrefHeight(754);
            outerRow.setMaxHeight(754);
        }
        GridPane.setValignment(qualityCanvas, javafx.geometry.VPos.TOP);
        applyQualityRightPanelLayout(false);
        ...
        ChangeListener<Number> redrawDefaultKmcgImage = (obs, oldVal, newVal) -> {
            if (kmcgdata.isEmpty()) {
                KMCG_Processing.showDefaultImage(KMCGCanvas);
            }
        };
        KMCGCanvas.widthProperty().addListener(redrawDefaultKmcgImage);
        KMCGCanvas.heightProperty().addListener(redrawDefaultKmcgImage);
        ...
        Quality_Processing.installAssemblyBarHover(qualityAssemblyCanvas);

//  质量图初始化显示
        if (targetData != null && !targetData.isEmpty()) {
            int initialRow = 0;
            int maxRows = targetData.size();
            configureQualitySlider(maxRows - 1);
    ...
    private void configureQualitySlider(int maxRowIndex) {
        qualitySlider.setMin(0);
        qualitySlider.setMax(Math.max(0, maxRowIndex));
        qualitySlider.setMajorTickUnit(40);
        qualitySlider.setValue(0);
    }

    private static final double QUALITY_LINE_ROW_WITH_BAR = 318;
    private static final double QUALITY_FITTING_ROW_WITH_BAR = 318;
    private static final double QUALITY_ASSEMBLY_ROW = 118;
    private static final double QUALITY_LINE_CANVAS_WITH_BAR = 312;
    private static final double QUALITY_FITTING_CANVAS_WITH_BAR = 312;
    private static final double QUALITY_ASSEMBLY_CANVAS = 115;
    private static final double QUALITY_EXTRA_CHART_HEIGHT = QUALITY_ASSEMBLY_ROW / 2.0;

    private void applyQualityRightPanelLayout(boolean showAssemblyBar) {
        if (qualityRightGrid == null || qualityRightGrid.getRowConstraints().size() < 3) {
            return;
        }

        double lineRowHeight = showAssemblyBar
                ? QUALITY_LINE_ROW_WITH_BAR
                : QUALITY_LINE_ROW_WITH_BAR + QUALITY_EXTRA_CHART_HEIGHT;
        double fittingRowHeight = showAssemblyBar
                ? QUALITY_FITTING_ROW_WITH_BAR
                : QUALITY_FITTING_ROW_WITH_BAR + QUALITY_EXTRA_CHART_HEIGHT;
        double lineCanvasHeight = showAssemblyBar
                ? QUALITY_LINE_CANVAS_WITH_BAR
                : QUALITY_LINE_CANVAS_WITH_BAR + QUALITY_EXTRA_CHART_HEIGHT;
        double fittingCanvasHeight = showAssemblyBar
                ? QUALITY_FITTING_CANVAS_WITH_BAR
                : QUALITY_FITTING_CANVAS_WITH_BAR + QUALITY_EXTRA_CHART_HEIGHT;

        RowConstraints lineRow = qualityRightGrid.getRowConstraints().get(0);
        lineRow.setMinHeight(lineRowHeight);
        lineRow.setPrefHeight(lineRowHeight);
        lineRow.setMaxHeight(lineRowHeight);

        RowConstraints fittingRow = qualityRightGrid.getRowConstraints().get(1);
        fittingRow.setMinHeight(fittingRowHeight);
        fittingRow.setPrefHeight(fittingRowHeight);
        fittingRow.setMaxHeight(fittingRowHeight);

        RowConstraints assemblyRow = qualityRightGrid.getRowConstraints().get(2);
        if (showAssemblyBar) {
            assemblyRow.setMinHeight(QUALITY_ASSEMBLY_ROW);
            assemblyRow.setPrefHeight(QUALITY_ASSEMBLY_ROW);
            assemblyRow.setMaxHeight(QUALITY_ASSEMBLY_ROW);
        } else {
            assemblyRow.setMinHeight(0);
            assemblyRow.setPrefHeight(0);
            assemblyRow.setMaxHeight(0);
        }

        if (qualityAssemblyPane != null) {
            qualityAssemblyPane.setVisible(showAssemblyBar);
            qualityAssemblyPane.setManaged(showAssemblyBar);
        }
        if (qualityAssemblyCanvas != null) {
            qualityAssemblyCanvas.setHeight(showAssemblyBar ? QUALITY_ASSEMBLY_CANVAS : 0);
        }
        if (qualityLineCanvas != null) {
            qualityLineCanvas.setHeight(lineCanvasHeight);
        }
        if (qualityFittingCanvas != null) {
            qualityFittingCanvas.setHeight(fittingCanvasHeight);
        }
        if (qualityFittingPane != null) {
            qualityFittingPane.setPrefHeight(fittingRowHeight);
            qualityFittingPane.setMaxHeight(fittingRowHeight);
        }
    }

    private void refreshQualityRightCharts(int rowIndex) {
        boolean showAssemblyBar = Quality_Processing.hasQualityAssemblyBarData();
        applyQualityRightPanelLayout(showAssemblyBar);
        ...
        if (targetData != null && !targetData.isEmpty() && rowIndex >= 0 && rowIndex < targetData.size()) {
            Quality_Processing.drawQualityFittingCanvas(qualityFittingCanvas, targetData, rowIndex);
        } else {
            clearCanvas(qualityFittingCanvas);
        }

        Quality_Processing.drawQualityAssemblyCanvas(qualityAssemblyCanvas);
    }

    private void restoreDefaultKmcgImage() {
        KMCG_Processing.showDefaultImage(KMCGCanvas);
        Platform.runLater(() -> {
            KMCG_Processing.showDefaultImage(KMCGCanvas);
            Platform.runLater(() -> KMCG_Processing.showDefaultImage(KMCGCanvas));
        });
    }
    ...
    private void resetViewerState() {
        ...
        tabPane.getTabs().removeIf(tab ->
                Boolean.TRUE.equals(tab.getProperties().get("dynamicTab")) ||
                        Boolean.TRUE.equals(tab.getProperties().get("dynamicInfTab")) ||
        ...
        clearCanvas(qualityLineCanvas);
        clearCanvas(qualityFittingCanvas);
        clearCanvas(qualityAssemblyCanvas);
        applyQualityRightPanelLayout(false);

        KMCG_Processing.clearKmcgCanvasMouseHandlers(KMCGCanvas);
        ...
        restoreDefaultKmcgImage();
    }

    @FXML
    public void handleOpenFile() {
        FileChooser fileChooser = new FileChooser();
        fileChooser.setTitle("Open KMCG file");
        ...
        if (owner == null && KMCGCanvas != null && KMCGCanvas.getScene() != null) {
            owner = (Stage) KMCGCanvas.getScene().getWindow();
        }

        File file;
        KMCG_Processing.setHoverTooltipsSuspended(true);
        try {
            file = fileChooser.showOpenDialog(owner);
        } finally {
            KMCG_Processing.setHoverTooltipsSuspended(false);
        }
        ...
        if (file != null) {
            ...
            if (loadedData == null || loadedData.isEmpty()) {
                resetViewerState();
                showAlert("Invalid KMCG file", "The selected file is not a valid KMCG file or contains no data.");
                restoreDefaultKmcgImage();
                return;
            }
            tabPane.getTabs().removeIf(tab ->
            ...
            createExtensionDynamicTabsInOrder(loadedData, totalKmcgSum);
            ...
            if (!loadedData.isEmpty()) {
                ...
                if (!targetData.isEmpty()) {
                    configureQualitySlider(targetData.size() - 1);
                    qualitySlider.setDisable(false);
                    rowLabel.setText("Row: 0");
                    qualityFittingPane.setVisible(true);

                    Quality_Processing.drawQualityOnCanvas(qualityCanvas, targetData);
                    refreshQualityRightCharts(initialRow);
                } else {
                    clearCanvas(qualityCanvas);
                    clearCanvas(qualityLineCanvas);
                    clearCanvas(qualityFittingCanvas);
    ...
    public void handleAbout(ActionEvent event) {
        ...
        alert.setContentText("Version: 3.6\n\n" +
                "Last Updated: " + BuildInfo.getLastUpdated() + "\n\n" +
    ...
    private boolean isGcPercentageTab(String tabName) {
        return tabName != null && tabName.equalsIgnoreCase("GCpercentage");
    }

    private void createExtensionDynamicTabsInOrder(List<List<Integer>> kmcgdata, Long totalKmcgSum) {
        tabPane.getTabs().removeIf(tab ->
                Boolean.TRUE.equals(tab.getProperties().get("dynamicTab")) ||
                        Boolean.TRUE.equals(tab.getProperties().get("dynamicInfTab")) ||
                        Boolean.TRUE.equals(tab.getProperties().get("dynamicGCTab"))
        ...
        for (KMCG_Processing.ExtensionBlockRef block : KMCG_Processing.extensionBlockOrder) {
            if ("inf".equals(block.mode)) {
                List<List<Integer>> infData = KMCG_Processing.infDataMap.get(block.tagName);
                if (infData != null) {
                    System.err.println("KMCG UI: creating # tab '" + block.tagName + "'");
                    addInfDynamicTab(block.tagName, infData, kmcgdata, totalKmcgSum);
                }
            } else if ("prm".equals(block.mode)) {
                List<List<Integer>> scaffoldData = scaffolddataMap.get(block.tagName);
                if (scaffoldData != null) {
                    addScaffoldDynamicTab(block.tagName, scaffoldData, kmcgdata, totalKmcgSum);
                }
            }
        }
    }

    private void addInfDynamicTab(String tabName, List<List<Integer>> infData,
                                 List<List<Integer>> kmcgdata, Long totalKmcgSum) {
        addScaffoldStyleExtensionTab(tabName, infData, kmcgdata, totalKmcgSum,
                KMCG_Processing.infBlockLabelMap.getOrDefault(tabName, ""), "dynamicInfTab", true, true);
    }

    private void addScaffoldDynamicTab(String tabName, List<List<Integer>> tabScaffoldData,
                                       List<List<Integer>> kmcgdata, Long totalKmcgSum) {
        addScaffoldStyleExtensionTab(tabName, tabScaffoldData, kmcgdata, totalKmcgSum,
                KMCG_Processing.scaffoldBlockLabelMap.getOrDefault(tabName, ""), "dynamicTab",
                false, false);
    }

    private void addScaffoldStyleExtensionTab(String tabName, List<List<Integer>> tabMatrixData,
                                             List<List<Integer>> kmcgdata, Long totalKmcgSum,
                                             String displayLabel, String tabPropertyKey,
                                             boolean mode0UsesLogTransform, boolean showModeSwitchButton) {
            int tabMaxValidValue = Scaffolds_Processing.findMaxValue(tabMatrixData);
            List<List<Integer>> tabSaturationData = Scaffolds_Processing.applySaturation(tabMatrixData, tabMaxValidValue);
            int tabScaffoldSum = 0;
            for (List<Integer> row : tabMatrixData) {
                for (Integer value : row) {
                    if (value != null) {
            ...
            TabData tabData = new TabData(tabMatrixData, tabMaxValidValue, tabSaturationData, tabScaffoldSum,
                    displayLabel);
            Scaffolds_Processing processor = "dynamicInfTab".equals(tabPropertyKey)
                    ? new InfScaffolds_Processing()
                    : new Scaffolds_Processing();
            processor.setEnableHoverTooltips(true);
            processor.setKmcgLayoutOptions(true, true);
            processor.setMode0UsesLogTransform(mode0UsesLogTransform);

            Tab newTab = new Tab(tabName);
            newTab.setClosable(false);
            newTab.getProperties().put(tabPropertyKey, true);
            // 创建 Tab 内容
            HBox hbox = new HBox();
            StackPane stackPane = new StackPane();
            AnchorPane anchorPane = new AnchorPane();
            Canvas canvas = new Canvas(1300.0, 820.0);
            ...
            if (showModeSwitchButton) {
                ...
                switchFormatButton.setVisible(false);
                switchFormatButton.setManaged(false);
            }
// 添加颜色区间设置按钮
            ...
            colorRangeButton.setVisible(!mode0UsesLogTransform);
            colorRangeButton.setManaged(!mode0UsesLogTransform);
            if (showModeSwitchButton) {
            // 切换按钮点击事件
            ...
            try {
                ...
                System.err.println("Extension tab draw failed for '" + tabName + "': " + ex.getMessage());
                ex.printStackTrace();
            }
    }
    ...
    public static class TabData {
        ...
        public TabData(List<List<Integer>> scaffolddata, int maxValidvalue,
                       List<List<Integer>> saturationdata, int totalScaffoldSum, String displayLabel) {
