...
public class MainController {
    ...
    public void handleSearchButton() {
        ...
        // 监听模式切换 - 不再清空已有输入
        inputModeGroup.selectedToggleProperty().addListener((obs, oldVal, newVal) -> {
            // 这里不再清空内容，只是改变新增行的模式
        });
...
// 计算总KMCG sum
            ...
            if (!kmcgdata.isEmpty()) {
                ...
                qualitySlider.setValue(0); // 初始为第一行
                rowLabel.setText("Row: 0");
    ...
    private void showFileDetails() {
        ...
        Label Kmer_length = new Label("Kmer lengths: " + kmerlength );
        Label Kmer_unitSize = new Label("Kmer Unit Size: " + Unitsize );
        vbox.getChildren().addAll(filenameLabel, heightLabel, widthLabel, Kmer_length, Kmer_unitSize);
