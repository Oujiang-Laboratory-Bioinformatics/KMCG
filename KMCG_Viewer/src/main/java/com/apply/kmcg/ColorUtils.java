...
public class ColorUtils {
    ...
    public static Color getUD2ColorWithCustomRange(double count, double maxres,
        ...
        // 3. 处理中间范围的值
            ...
            else if (mappedPercentage <= 35) {
                // 黄色到橙色渐变（20-35%）
                return interpolateColor(Scheme[4], Scheme[6], mappedPercentage, 20, 35);
            }
            else if (mappedPercentage <= 50) {
                // 橙色渐变（35-50%）
                return interpolateColor(Scheme[6], Scheme[8], mappedPercentage, 35, 50);
            }
            else {
                // 橙色到红色渐变（50-60%）
