...
//            final double Y_OFFSET_FACTOR = 2;
//
//            // 初始化
//            GraphicsContext gc = dataCanvas.getGraphicsContext2D();
...
//            double normalHeight = (NORMAL_SECTION_END + 1) * CELL_HEIGHT;
//            double stretchedHeight = (rowCount - NORMAL_SECTION_END - 1) * CELL_HEIGHT * MAGNIFICATION;
//            double totalHeight = normalHeight + stretchedHeight;
//            double offsetX = (dataCanvas.getWidth() - totalWidth) / 10;
//            double offsetY = (dataCanvas.getHeight() - totalHeight) / Y_OFFSET_FACTOR;
//
//            // 绘制坐标轴
//            gc.setStroke(Color.BLACK);
//            gc.setLineWidth(1);
//            gc.strokeLine(offsetX, offsetY + totalHeight, offsetX + totalWidth, offsetY + totalHeight); // X轴
//            gc.strokeLine(offsetX, offsetY, offsetX, offsetY + totalHeight); // Y轴
//
//            // 绘制数据矩形
//            for (int y = 0; y < rowCount; y++) {
//                double yPos, rowHeight;
//                if (y <= NORMAL_SECTION_END) {
//                    yPos = offsetY + (NORMAL_SECTION_END - y) * CELL_HEIGHT + stretchedHeight;
//                    rowHeight = CELL_HEIGHT;
//                } else {
//                    double stretchBaseY = offsetY + stretchedHeight;
//                    double stretchOffset = (y - NORMAL_SECTION_END - 1) * CELL_HEIGHT * MAGNIFICATION;
//                    yPos = stretchBaseY - stretchOffset - CELL_HEIGHT * MAGNIFICATION;
...
//                        gc.setFill(color);
//                        gc.fillRect(offsetX + x * CELL_WIDTH, yPos, CELL_WIDTH, rowHeight);
//                    }
//                }
//            }
//            if (rowCount > NORMAL_SECTION_END) {
//                double y512Pos = offsetY + stretchedHeight;
//
//                gc.setStroke(Color.RED);
//                gc.setLineWidth(2);
//
//                // 替换箭头为整条红线
//                gc.strokeLine(offsetX, y512Pos, offsetX + totalWidth , y512Pos);
//            }
//
//            // 绘制X轴刻度
...
//                gc.strokeLine(xPos, offsetY + totalHeight, xPos, offsetY + totalHeight + 6);
//                gc.fillText(Integer.toString(x), xPos - 10, offsetY + totalHeight + 18);
//            }
//
//            if (colCount % 100 != 0) {
//                double xPos = offsetX + colCount * CELL_WIDTH;
//                gc.strokeLine(xPos, offsetY + totalHeight, xPos, offsetY + totalHeight + 6);
//                gc.fillText(Integer.toString(colCount), xPos - 10, offsetY + totalHeight + 18);
//            }
//
//
//
...
//                String label = "";
//
//                if (y <= NORMAL_SECTION_END && y % 100 == 0) {
//                    drawLabel = true;
//                    label = Integer.toString(y);
//                } else if (y >= NORMAL_SECTION_END) {
//                    drawLabel = true; // 每个拉长行都标注
//                }
//
//                if (drawLabel) {
//                    double yPos;
...
//                        // 正常部分
//                        gc.setStroke(Color.BLACK);
//                        gc.strokeLine(offsetX - 6, yPos, offsetX, yPos);
//                        gc.setFont(Font.font("Verdana", 10));
//                        gc.setFill(Color.BLACK);
//                        gc.fillText(label, offsetX - 38, yPos + 3);
//                    } else {
//                        // 放大部分：2^n 的刻度
...
//                        gc.setFill(Color.RED);
//                        gc.setFont(Font.font("Verdana", 10));
//                        gc.fillText("2", offsetX - 27, yPos + 3);  // 基数
//
//                        gc.setFont(Font.font("Verdana", 7));
//                        gc.fillText(String.valueOf(exponent), offsetX - 20, yPos - 2);  // 指数
//
//                        gc.setStroke(Color.BLACK);
//                        gc.setFill(Color.BLACK);
//                    }
...
public static void drawQualityOnCanvas(Canvas dataCanvas, List<List<Integer>> data) {
    ...
    // ✅ 填充白色背景，避免 Linux 渲染问题
    gc.setFill(Color.WHITE);
    gc.fillRect(0, 0, dataCanvas.getWidth(), dataCanvas.getHeight());
    ...
    if (rowCount > NORMAL_SECTION_END) {
        ...
        gc.setStroke(Color.RED);
        gc.setLineWidth(2);
        gc.strokeLine(offsetX, y512Pos, offsetX + totalWidth , y512Pos); // 红线代替箭头
    }
    ...
    for (int y = 0; y < rowCount; y++) {
        ...
        if (drawLabel || y >= NORMAL_SECTION_END) {
            double yPos;
            if (y <= NORMAL_SECTION_END) {
                yPos = offsetY + (NORMAL_SECTION_END - y) * CELL_HEIGHT + stretchedHeight;
            } else {
            ...
            if (y < NORMAL_SECTION_END) {
                ...
                int exponent = 9 + (y - NORMAL_SECTION_END); // 从2^9开始
                gc.setStroke(Color.RED);
                gc.strokeLine(offsetX - 10, yPos, offsetX, yPos);

                gc.setFill(Color.RED);
                gc.setFont(Font.font("Verdana", 10));
                gc.fillText("2", offsetX - 27, yPos + 3);

                gc.setFont(Font.font("Verdana", 7));
                gc.fillText(String.valueOf(exponent), offsetX - 20, yPos - 2);
