...
//                    gc.fillText(Integer.toString(y), offsetX - 30, yPos + 4);
    ...
    public static void drawQualityLineCanvas(Canvas dataCanvas) {
        ...
        gc.fillText("(%)", xStart - 10, 30);  //  Y 轴顶部的偏移量
        // 绘制折线
    ...
    public static void drawQualityBarChart(Canvas canvas, List<Integer> rowData, int rowIndex) {
        ...
        // Gaussian Fitting Curve (RED DASHED) — μ/σ from file Row(N+1)_Mean/Sigma when available
        double mu;
        double sigma;
        double[] fileParams = qualityGaussianByRow.get(rowIndex);
        if (fileParams != null && fileParams.length >= 2) {
            mu = fileParams[0];
            sigma = fileParams[1];
        } else {
            double[] params = estimateGaussianParams(rowData);
            mu = params[0];
            sigma = params[1];
        }
        ...
        // Row 0: show μ/σ/Overlap from file Row1_* but skip red curve (histogram ≠ Row1 fit)
        ...
        // μ, σ, Overlap display —— right top, left-aligned internally, black
        gc.setFill(Color.BLACK);
        ...
        double textRightX = canvasWidth - marginRight - 345;  // 整体靠右
        double baseY = marginTop + effectiveChartHeight * 0.22;        // 往下移动
        ...
        // μ
        gc.fillText(muText, textRightX, baseY);
        // σ
        gc.fillText(sigmaText, textRightX, baseY + muHeight + 6);

        // ===== Overlap % display
        double overlapSum = 0;
        for (int i = 0; i < barCount; i++) {
            double pdf = normalPDF(i, mu, sigma) * sum;
            double actual = rowData.get(i) != null ? rowData.get(i) : 0;
            overlapSum += Math.min(pdf, actual);
        }
        double overlapPercent = (sum > 0) ? (overlapSum / sum) * 100 : 0;

        String overlapText = String.format("Overlap = %.2f%%", overlapPercent);
        gc.fillText(overlapText, textRightX, baseY + muHeight + sigmaHeight + 12);
    }
