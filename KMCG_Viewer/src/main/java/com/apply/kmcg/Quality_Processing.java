...
//                    gc.fillText(Integer.toString(y), offsetX - 30, yPos + 4);
    ...
    public static void drawQualityBarChart(Canvas canvas, List<Integer> rowData) {
        ...
        for (int i = 0; i <= yTickCount; i++) {
            ...
            // 文本X,Y位置对齐刻度线垂直居中
            double textX = marginLeft - 10 - textWidth;
            double textY = y + textHeight / 4; // 修正文字垂直居中
        ...
        for (int i = 0; i < barCount; i += xTickInterval) {
            ...
            Text textNode = new Text(xLabel);
            textNode.setFont(gc.getFont());
            double textWidth = textNode.getLayoutBounds().getWidth();
            gc.fillText(xLabel, x - textWidth / 2, canvasHeight - marginBottom + 15 + textNode.getLayoutBounds().getHeight() / 2);
        }
    }
