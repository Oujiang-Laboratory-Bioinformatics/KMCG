...
import static com.apply.kmcg.KMCG_Processing.kmerlength;
...
//长度
    ...
    public static List<Map.Entry<Character, Integer>> parseData(String data) {
        ...
        for (String item : dataItems) {
            // 忽略前kmerlength 31个字符，从第32个字符开始处理
            if (item.length() <= kmerlength) continue; // 确保数据足够长
            String trimmedItem = item.substring(kmerlength);
            // 每四个字符处理一次
...
// 测试打印字典内容 把类似于(177, 108): AGT...!007!007! (2, 416): TCT...!007!007!都放在一起
    ...
    public static Map<String, Integer> parseAndCount(int rowIndex, int columnIndex) {
        ...
        for (String value : resultData) {
            ...
            for (String item : dataItems) {
                if (item.length() < kmerlength) continue; // 确保数据项足够长

                String name = item.substring(0, kmerlength); // 前31个字符作为名字
                String data = item.substring(kmerlength);   // 剩下的部分作为数据内容
