...
void normalize_data(const std::vector<unsigned int>& data_row, std::vector<double>& normalized_row) {
...
size_t find_initial_mean(const std::vector<double>& line, double threshold) {
...
bool appendWordToCell(std::vector<std::string>& cell, const std::string& word)
{
...
{	std::vector<std::chrono::steady_clock::time_point> time_points;
	...
	{	std::cout  << argv[i] << "    i="<<i<<std::endl;
		...
		{	
			...
			else if(strncmp(argv[i], "-xstep", 6) == 0)
                xstep = atoi(&argv[i][6]);
            else if(strncmp(argv[i], "-ystep", 6) == 0)
                ystep = atoi(&argv[i][6]);
            else if(strncmp(argv[i], "-p", 2) == 0)
                p = atoi(&argv[i][2]);      
            else if(strncmp(argv[i], "-xbox", 5) == 0)
                xbox = atoi(&argv[i][5]);   	
            else if(strncmp(argv[i], "-ybox", 5) == 0)
                ybox = atoi(&argv[i][5]);  		
			else if(strncmp(argv[i], "-t", 2) == 0)
                s = atoi(&argv[i][2]);		
			else if(strncmp(argv[i], "-scr",4) == 0)
                CHR = std::string(&argv[i][4]);	
			else if(strncmp(argv[i], "-s", 2) == 0)
                sex = std::string(&argv[i][2]);						     			     
		}
		else
			break;
	}
	...
	std::vector<std::vector<int>> kmccount(sizeofy, std::vector<int>(sizeofx));
	std::vector<std::vector<std::vector<std::string>>> kmer2chr(sizeofy, std::vector<std::vector<std::string>>(sizeofx));
	vector<vector<int>> count(sizeofy, vector<int>(sizeofx));
	vector<vector<uint32>> countstep1(521, vector<uint32>(130));
	std::vector<std::vector<int>> kmernum(ybox, std::vector<int>(xbox, 0));
	...
	std::vector<std::string> SelectedchrArray; 
	// std::string jobname = (slurmJobID != nullptr) ? slurmJobID : std::to_string(getpid());
	// 初始化随机种子
	...
	std::stringstream Selectedchr(CHR); 
	std::string item;
	while (getline(Selectedchr, item, ',')) {
		SelectedchrArray.push_back(item);
	}
	std::vector<std::string> scrsequence(SelectedchrArray.size());//循环生成三文件的kmc
	std::string folderName = jobname+"kmcfor3kmc";
	std::vector<std::vector<std::vector<int>>> kmc3count(  
		SelectedchrArray.size(),                                            
	...
	for (size_t i = 0; i < SelectedchrArray.size(); ++i) 
	{
		const auto& chrRange = SelectedchrArray[i];
		size_t colonPos = chrRange.find(':');
		size_t dashPos = chrRange.find('-');
		std::string chrName = chrRange.substr(0, colonPos);
		// 提取 start 和 end 的字符串部分
		std::string startStr = chrRange.substr(colonPos + 1, dashPos - colonPos - 1);
		std::string endStr = chrRange.substr(dashPos + 1);
		// 转换为浮点数（double 类型）
		double start = std::stod(startStr);
		double end = std::stod(endStr);
		scrsequence[i] = extractSequence(filepath1, chrName, start, end);
		if (!scrsequence[i].empty()) 
		{			
			std::string tempFileName =folderName + "/" +jobname+"+"+chrName +":"  + std::to_string(start) + "-" + std::to_string(end)+".fa";
			std::string tempDirName = folderName + "/" +jobname+"+"+chrName + ":"+ std::to_string(start) + "-" + std::to_string(end);
			std::ofstream outFile(tempFileName);
			if (!outFile.is_open()) {
				std::cerr << "Unable to create file: " <<tempFileName<< std::endl;
				return 1;
			}
			outFile << ">"+chrName+":"  + std::to_string(start) + "-" + std::to_string(end)+"\n";
			outFile << scrsequence[i];
			outFile.close();
			std::ifstream filetemp(tempFileName);
			if (!filetemp.is_open()) {
				std::cerr << "Unable to open file: " <<tempFileName<< std::endl;
				return 1;
			}
			std::string command = 
				"kmc -k31 -t10 -m190 -ci1 -fm -v " +
				"\"" + tempFileName + "\" " +   
				"\"" + tempDirName + "\" " +    
				"./ | awk '{print $4}'";        
			...
			if (std::remove(tempFileName.c_str()) != 0) {
				std::cerr << "Error deleting file " << tempFileName << std::endl;
			} else {
				std::cout << "File " << tempFileName << " successfully deleted." << std::endl;
			}
		} 
		else 
		{
			std::cout << "Failed to extract sequence for " << chrName << "\n";
		}
	}
	...
	fprintf(out_file,"KMCG1\t%u\t%u\t",ybox,xbox);
	fprintf(out_file,"%ld\n",SelectedchrArray.size());
	for (size_t i = 0; i < sortedChr1.size(); ++i) {
		fprintf(out_file,"%s\t",sortedChr1[i].c_str());
	}
	for (size_t i = 0; i < sortedChr2.size(); ++i) {

		fprintf(out_file,"%s",sortedChr2[i].c_str());
		if ((i+1)%100!=0){
	...
	for (size_t i = 0; i < sortedChr1.size(); ++i) {
		...
		fprintf(out_file, "%d\t", (int)ceil((double)sortedChrnumber1[i] / tt));
	...
	{
		fprintf(out_file,"%zu",(sortedChr2.size()%100));
		...
		setvbuf(out_file, NULL ,_IOFBF, 1 << 24);
			...
			else
			{
				...
				if (readtime%p==0)
				{
					...
					if (is_kmer_found) 
					{
						...
						count[0][y]++;
						appendWordToCell(array[0][y],kmer_object.to_string().c_str());
			...
			else
			{
				...
				if (readtime%p==0)
				{
					...
					if (is_kmer_found) 
					{
						...
						count[y][x]++;   
						appendWordToCell(array[y][x],kmer_object.to_string().c_str());   
						...
						count[y][0]++;
						appendWordToCell(array[y][0],kmer_object.to_string().c_str());
			...
			for (int t = 0; t < num_strings; ++t) {
					...
					exit(EXIT_FAILURE);
																	...
																	for (uint32_t i = 0; i < ROWS; ++i)
																	{
																		...
																		{	
																			...
																			{	
																				cnt = FMIdx[t]->count(const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(word.c_str())), strlen(word.c_str()));
																				if (cnt>0)
																				{
																					result = FMIdx[t]->locate(const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(word.c_str())), strlen(word.c_str()), &matches);
																					...
																					cnt = FMIdx[t]->count(const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(str_word.c_str())), str_word.length());
																					if (cnt>0)
																					{
																						result = FMIdx[t]->locate(const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(str_word.c_str())), str_word.length(), &matches);
																						std::string name(chrnamechr[t]);
																						std::string target = name;
																						auto it = std::find(sortedChr1.begin(), sortedChr1.end(), target);
																						auto itt= std::find(sortedChr2.begin(), sortedChr2.end(), target);
				...
				for (size_t i = 0; i < countstep1.size(); ++i) 
				{
					...
					{
						fprintf(out_file, "%d\t", countstep1[i][j]); 
					}
					fprintf(out_file, "\n");
				}
    ...
    for (size_t i = 1; i < countstep1.size(); ++i) {
        ...
        if (i == 1) {
            // 检查第一行是否总和全零
            double row_1_sum = std::accumulate(countstep1[1].begin() + 1, countstep1[1].end(), 0.0);
            if (row_1_sum <= 0) {
                throw std::runtime_error("Error: The total sum of data in the row_1 is <= 0.");
            }

            double init_mean = find_initial_mean(normalized_vectors[i], 0.5);
            ...
            double row_sum = std::accumulate(countstep1[i].begin() + 1, countstep1[i].end(), 0.0);
	...
	size_t initial_mean = find_initial_mean(normalized_row, 0.5);
	...
	std::cerr << "Error: sumforki0+sumforki==0" << std::endl;
			...
			std::string CHRnames[SelectedchrArray.size()];
			if (!kmer_data_base.OpenForRA(input_file_name1))
			{   
				printf("OpenForRA1 failed");
				print_info();
				return EXIT_FAILURE ;
			}
			else
			...
			else
			{
				...
				std::queue<uint32_t> num;
				uint64 counter1;
				uint64 counterfor3kmc;
				int y=0;
				int sizeofY =xbox;
				if(CHR.size())
				{
				int outputchrnamenum=0;			
				DIR* dir = opendir(folderName.c_str());
				if (dir == nullptr) {
					std::cerr << "Failed to open directory: " << folderName << std::endl;
					return 1;
				}
				struct dirent* entry;
				while ((entry = readdir(dir)) != nullptr) 
				{			
					// 跳过 "." 和 ".."，以及不以 .kmc_suf 结尾的文件
					std::string fileName = entry->d_name;
					if (fileName == "." || fileName == ".." || 
						(fileName.size() < 8 || fileName.substr(fileName.size() - 8) != ".kmc_suf")) {
						continue;
					}
					else
					{
					...
					CHR=realpath(filePath.c_str(), nullptr);
					// 查找最后一个 '.' 的位置
					size_t lastDotPos1 = CHR.find_last_of('.');
					// 如果找到了 '.' 并且它后面的内容是 "kmc_suf"
					if (lastDotPos1 != std::string::npos && CHR.substr(lastDotPos1) == ".kmc_suf") 
					{
						// 删除从最后一个 '.' 开始到字符串末尾的部分
						CHR = CHR.substr(0, lastDotPos1);
					}
					size_t plus_pos = CHR.find_last_of('+');
					std::string extracted="";
					if (plus_pos != std::string::npos) {
						// 2. 截取 '+' 之后的内容
						extracted = CHR.substr(plus_pos + 1);
						
					} else {
					}
					CHRnames[outputchrnamenum]=extracted;
					// CHR = tempDirName;
					if (!kmer_data_basefor3kmc.OpenForRA(CHR))
					{   
						printf("OpenForRAfor3kmc failed");
						print_info();
						return EXIT_FAILURE ;
					}
					else
					{
                        ...
                        if (readtime%p==0)
                        {
						bool is_kmer_foundfor3kmc = kmer_data_basefor3kmc.CheckKmer(kmer_object, counterfor3kmc);
						if (is_kmer_foundfor3kmc)
						{
								...
								else 
								{
										...
										kmc3count[outputchrnamenum][0][y]++;
								}
		...
		{			
			...
			if (lastDotPos1 != std::string::npos && CHR.substr(lastDotPos1) == ".kmc_suf") 
			{
				...
				CHR = CHR.substr(0, lastDotPos1);
					...
					if (!kmer_data_basefor3kmc.OpenForRA(CHR))
					{   
						printf("OpenForRAfor3kmc1 failed");
						print_info();
						return EXIT_FAILURE ;
					}
					else
					{
						...
						if (is_kmer_foundfor3kmc)
						{
								...
								if (is_kmer_found) 
								{
									...
									kmc3count[outputchrnamenum][y][x]++;
								}			
								else {
									if (counter <=ystep*(ybox-2) )
										{	                                                                   
									...
									kmc3count[outputchrnamenum][y][0]++;
								}
						}
                        }
						readtime++;
...
{	
		...
		fprintf(out_file, "%s", CHRnames[i].c_str());
		fprintf(out_file,"\n");

		for (int j = 0; j < sizeofy; ++j) 
			{
				for (int k = 0; k <sizeofx ; ++k) {
					fprintf(out_file, "%d\t", kmc3count[i][j][k]); 
				}
				fprintf(out_file, "\n");
			}
			fprintf(out_file, "\n");
