...
input_file_name1=tempDirNameofinput_file_name1;
	...
	// // 打开gz文件（注意模式使用"wb"或"wb9"提高压缩级别）
	// gzFile out_file = gzopen("output.txt.gz", "wb9");
				...
				delete FMIdx[t];
							...
							for (uint32_t i = 0; i < ROWS; ++i)
							{
								...
								{	
									...
									{
										gzprintf(out_file,"%s ",word.c_str());
										ttt++;
									}
								gzprintf(out_file,"\t");
								}
	...
	else
	{
		...
		if(gc!=0)
		{
			...
			else
			{
				...
				if(CHR.size())
				{
					...
					bool isCHRfa = check_fasta_extension(CHR);
					if (isCHRfa) 
						{
							std::cout << 1 << std::endl;  // 符合条件时输出1
						// 生成临时目录名：folderName + 文件名（不含路径和扩展名）
						size_t lastSlashPos = CHR.find_last_of("/\\");  // 查找最后一个路径分隔符
						std::string fileName = (lastSlashPos != std::string::npos) ? 
												CHR.substr(lastSlashPos + 1) : 
												CHR;
						...
						std::string tempDirNameof3kmc = folderName + "/" +baseName;  // 拼接结果
						...
						commandStream << "kmc -k" << kmerlength 
							...
							std::string command = commandStream.str();
									...
									if (ret == 0) {
										// 成功逻辑
										std::cout << "'" << tempDirNameof3kmc << "' 创建成功.\n";
									} else {
										std::cerr << "Unable to call kmc."<< std::endl;// 失败逻辑
									}
						CHR=tempDirNameof3kmc;
						} 
		...
		gzprintf(out_file, "%s", baseName.c_str());
		gzprintf(out_file,"\n");

		for (int j = 0; j < sizeofy; ++j) 
			{
				for (int k = 0; k <sizeofx ; ++k) {
					gzprintf(out_file, "%d\t", kmc3count[0][j][k]); 
				}
				gzprintf(out_file, "\n");
			}
			gzprintf(out_file, "\n");
...
void print_info(void)
{
	std::cout << "KMCG version 1.0-beta.\n"
