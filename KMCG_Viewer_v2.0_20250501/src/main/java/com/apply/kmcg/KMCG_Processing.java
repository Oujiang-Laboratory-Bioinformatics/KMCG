...
//                    }
    ...
    public static void handleQuadrilateralSelection() {
        ...
        if (!pointsInside.isEmpty()) {
            manualPoints.clear();
            pointsString.append("Points inside the quadrilateral:\n");
            int count = 0;
        ...
        // 如果有手动添加的点
        if (!MainController.manualPoints.isEmpty()) {
            quadrilateralPoints.clear();

            pointsString.append("\nManually added points:\n");
            int count = 0;

            for (double[] point : MainController.manualPoints) {
                pointsString.append(String.format("(%d, %d) ", (int) point[0], (int) point[1]));
                count++;

                if (count % 7 != 0 && count < MainController.manualPoints.size()) {
                    pointsString.append(", ");
                }
...
//    画四边形
    ...
    public static void showRegion5Coordinates(Canvas dataCanvas, List<List<Integer>> data,
        ...
        int dataX = xCoord;  // x坐标从1开始，对应data的第2列（索引1）
        int dataY = displayY; // 使用displayY而不是logicalY，确保数据索引不变

        // 确保索引在data范围内（因为data是302x1002，region5是300x1000）
        dataX = Math.min(dataX, 1000);  // 最大1000（对应data的索引1001）
        dataY = Math.min(dataY, 300);    // 最大300（对应data的索引301）
        ...
        currentTooltip.show(dataCanvas, mouseX + 120, mouseY + 50);
    }
...
//                tooltipText = String.format("Count: %d", value);
            ...
            currentTooltip.show(dataCanvas, mouseX + 150 , mouseY + 70);  // 加上偏移量，让 Tooltip 不会与鼠标重叠
        }
    }
